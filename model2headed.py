...
class EnetDecoder(nn.Module):
    def __init__(self, params, nclasses1, nclasses2, encoder):
        super().__init__()
    ...
    def forward(self, input):
        ...
        output1 = self.output_conv1(output)
        output2 = self.output_conv2(output)
...
class Model2Headed(nn.Module):
    def __init__(self, nclasses1, nclasses2, mlp_num_layers,use_gpu):
        super().__init__()

        self.encoder = EnetEncoder(ENCODER_PARAMS)
        self.gnn = EnetGnn(mlp_num_layers,use_gpu)
        self.decoder = EnetDecoder(DECODER_PARAMS, nclasses1, nclasses2, self.encoder)
