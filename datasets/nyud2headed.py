...
class Dataset(Dataset):
    def __init__(self, flip_prob=None, crop_type=None, crop_size=0):
        ...
        label_images_fr = np.array(f['labels'][0:N_IMAGES])
        our_label_images_fr = np.array(f['our_labels'][0:N_IMAGES])
        self.label_names = np.array(['<UNK>'] + self.get_names(f))
        self.label2_names = np.array(['<UNK>'] + self.get_names(f, field='our_names'))
        f.close()

        self.rgb_images = rgb_images_fr
        self.label_images = label_images_fr
        self.label2_images = our_label_images_fr
    ...
    def __getitem__(self, idx):
        ...
        label2 = self.label2_images[idx].astype(np.float32)
        label[label >= len(self.label_names)] = 0
        label2[label2 >= len(self.label2_names)] = 0
        ...
        if self.crop_type is not None and self.crop_size > 0:
            ...
            if max_margin == 0:  # crop is original size, so nothing to crop
                ...
                label2 = label2[max_margin // 2:-max_margin // 2, max_margin // 2:-max_margin // 2]
                ...
                label2 = label2[y_:y_ + self.crop_size, x_:x_ + self.crop_size]
        ...
        return rgb_hha, label, xy, label2
